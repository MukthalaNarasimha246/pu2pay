...
def process_pdfs(root_input, root_output, overwrite=True, db_name=None):
    print('Processing started the with pdfs')
    results = []
...
def process_images(root_input, root_output,db_name, overwrite=True,):
    results = []
    ...
    for dirpath, dirnames, filenames in os.walk(root_input):
        ...
        try:
            insert_id = insert_pdf_conversion(os.path.basename(dirpath), 0, len(image_files), status,db_name)
...
def insert_pdf_conversion(subfolder, num_pdfs, num_images, status,db_name):
...
async def upload_file( request: Request, file: UploadFile = File(...)):
    ...
    try:
        ...
        if file.filename.lower().endswith(ALLOWED_EXTENSIONS):
            print('Coming the pdf block')
            process_result = process_pdfs(root_input, root_output,db_name)
            process_type = "PDF conversion"
        else:
            process_result = process_images(root_input, root_output,db_name)
...
def insert_record(table_name: str, record: dict, request: Request):
...
def get_records_by_batch_id(table_name: str, batch_id: str, request: Request):
    if table_name not in TABLE_MODEL_MAPPING:
        raise HTTPException(status_code=400, detail=f"Table {table_name} not supported yet.")
...
def get_records_by_po_id(table_name: str, po_id: str, request: Request):
    if table_name not in TABLE_MODEL_MAPPING:
        raise HTTPException(status_code=400, detail=f"Table {table_name} not supported yet.")
...
def get_records_by_po_id(table_name: str, invoice_id: str, request: Request):
    if table_name not in TABLE_MODEL_MAPPING:
        raise HTTPException(status_code=400, detail=f"Table {table_name} not supported yet.")
...
def get_records_by_po_id(table_name: str, grn_id: str, request: Request):
    if table_name not in TABLE_MODEL_MAPPING:
        raise HTTPException(status_code=400, detail=f"Table {table_name} not supported yet.")
...
def get_invoice_full_details(invoice_number: str, request: Request):
    conn = None
    try:
        # conn = get_db_connection()
        db_name = get_db_name_from_token(request)
...
def get_invoice_full_details(po_number: str, request: Request):
    conn = None
    db_name = get_db_name_from_token(request)
...
async def get_pdf_conversion_data(request: Request):
    # conn = get_db_connection()
    # 
    db_name = get_db_name_from_token(request)
    print("++++++++++++++++",db_name)
...
async def get_classification_details_table_data(request: Request):
    # conn = get_db_connection()
    db_name = get_db_name_from_token(request)
    print(db_name)
    conn = get_psycopg2_connection(db_name)
...
async def get_classification_details_table_data(claim_id:str, request: Request):
    db_name = get_db_name_from_token(request)
    conn = get_psycopg2_connection(db_name)
    cursor = conn.cursor()
    try:
...
@app.put("/classification_details/{image}/{top_level}")
def update_classification_details_table_data(image: str, top_level: str, request: Request):
    db_name = get_db_name_from_token(request)
    conn = get_psycopg2_connection(db_name)
    cur = conn.cursor(cursor_factory=RealDictCursor)  # <-- ensure dictionary results

    try:
        # 1️⃣ Check if the image exists
        cur.execute("SELECT * FROM image_classification_hypotus WHERE image = %s;", (image,))
        result = cur.fetchone()
        print(result)

        if not result:
            raise HTTPException(status_code=404, detail=f"No record found for image: {image}")

        claim_id = result["claim_id"]
        print("Found claim_id:", claim_id)

        # 2️⃣ Update top_label safely
        query = """
            UPDATE image_classification_hypotus
            SET top_label = %s,is_hitl = TRUE
            WHERE image = %s
            RETURNING claim_id, top_label;
        """
        cur.execute(query, (top_level, image))
        updated = cur.fetchone()
        conn.commit()

        if not updated:
            raise HTTPException(status_code=400, detail="No row updated — check your image or top_label column.")

        return {
            "message": f"✅ Classification updated successfully for claim_id: {claim_id}",
            "updated_data": {
                "claim_id": updated["claim_id"],
                "top_label": updated["top_label"]
            }
        }
        ...
        raise HTTPException(status_code=500, detail=f"Error updating classification: {str(e)}")
...
async def get_bbox(top_label:str, request: Request):
    db_name = get_db_name_from_token(request)
    conn = get_psycopg2_connection(db_name)
    cursor = conn.cursor()
    try:
...
def insert_invoice_full_details( request: Request, payload: Dict = Body(...)):
    conn = None
    try:
        db_name = get_db_name_from_token(request)
        conn = get_psycopg2_connection(db_name)
...
def insert_invoice_full_details(request: Request, payload: Dict = Body(...)):
    ...
    try:
        ...
        # Insert into invoice_details (bill_to & bill_to_address removed correctly)
        cur.execute("""
            INSERT INTO invoice_details (
                invoice_number, invoice_date, total_amount, po_ref
            ) VALUES (%s, %s, %s, %s)
            RETURNING invoice_id
        """, (
            invoice["invoice_number"],
        ...
        print(row,'rowing')
        if not row:
            raise HTTPException(status_code=500, detail="Failed to insert invoice (no ID returned)")
...
def update_invoice_full_details( request: Request, invoice_number: str, payload: Dict = Body(...)):
    conn = None
    try:
        db_name = get_db_name_from_token(request)
        conn = get_psycopg2_connection(db_name)
...
@app.get("/get_invoice_po_match/{invoice_id}")
def get_invoice_po_match(invoice_id: int, request: Request):
    try:
        db_name = get_db_name_from_token(request)
        conn = get_psycopg2_connection(db_name)
        cur = conn.cursor()
...
@app.get("/get_invoice_po_summary/{invoice_id}")
def get_invoice_po_summary(invoice_id: int, request: Request):
    try:
        db_name = get_db_name_from_token(request)
        conn = get_psycopg2_connection(db_name)
        cursor = conn.cursor()
...
def get_invoice_po_details(request: Request):
    conn = None
    try:
        db_name = get_db_name_from_token(request)
        conn = get_psycopg2_connection(db_name)
...
def get_invoice_po_details(invoice_id: int,request: Request):
    try:
        db_name = get_db_name_from_token(request)
        conn = get_psycopg2_connection(db_name)
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
...
def get_lineitem_summary(inv_id: int,request: Request):
    db_name = get_db_name_from_token(request)
    conn = get_psycopg2_connection(db_name)
    cur = conn.cursor()
...
def get_metadata_checklist(inv_id: int,request: Request):
    db_name = get_db_name_from_token(request)
    conn = get_psycopg2_connection(db_name)
    cur = conn.cursor()
...
@app.get("/combined_transaction_summary/{invoice_id}")
def get_combined_summary(invoice_id: int,request: Request):
    try:
        db_name = get_db_name_from_token(request)
        conn = get_psycopg2_connection(db_name)
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
...
def get_invoice_checklist(batch_id: str, request: Request ):
    try:
        # conn = get_db_connection()
        db_name = get_db_name_from_token(request)
        conn = get_psycopg2_connection(db_name)
...
def get_invoice_checklist(invoice_id: int,batch_id:str, request: Request ):
    try:
...
# --- GET API to fetch a progress_file by batch_id ---
@app.get("/progress-files/{batch_id}")
def get_progress_file(batch_id: str,request: Request):
    # conn = get_db_connection()
    db_name = get_db_name_from_token(request)
    conn = get_psycopg2_connection(db_name)
    cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    ...
    try:
        # Plain SQL query
        cur.execute(
            "SELECT * FROM public.progress_files WHERE batch_id = %s ORDER BY batch_id ASC",
            (batch_id,)
        )
        result = cur.fetchone()
        if not result:
            raise HTTPException(status_code=404, detail=f"Progress file with batch_id '{batch_id}' not found")
        return result
    finally:
        cur.close()
        cur.close()
...
def po_list_details(vendor_name: str, request: Request):
    db_name = get_db_name_from_token(request)
    conn = get_psycopg2_connection(db_name)
    cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    ...
    try:
        ...
        records = cur.fetchall()
        return records  # Already list of dicts due to RealDictCursor
...
def update_po_ref(data: UpdatePORefRequest,request: Request):
    try:
        db_name = get_db_name_from_token(request)
        conn = get_psycopg2_connection(db_name)  # rename for clarity
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
